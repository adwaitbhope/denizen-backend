...
def create_details_pdf(township):
    ...
    pdf.write(6, 'Click here',
              'https://www.google.com/maps/search/?api=1&query=' + str(township.lat) + ',' + str(township.lng))
    pdf.ln()
    pdf.ln()
...
def get_new_paytm_cust_id():
    ...
    while (Township.objects.filter(paytm_cust_id=random_str).count() != 0) and (
            User.objects.filter(paytm_cust_id=random_str).count() != 0):
        random_str = ''.join(random.sample(letters, length))
    return random_str
...
def register_existing_initiate(request):
    ...
    for i in range(wings_num):
        ...
        wing = Wing.objects.create(township=township, name=wing_name, floors=wing_floors,
                                   apts_per_floor=wing_apts_per_floor, naming_convention=wing_naming_convention)
    ...
    for i in range(amenities_num):
        ...
        amenities.append(Amenity(township=township, name=amenity_name, billing_rate=amenity_billing_rate,
                                 amt_time_period=amenity_amt_time_period, time_period=amenity_time_period,
                                 free_for_members=amenity_free_for_members))
    ...
    for i in range(admin_ids):
        ...
        users.append(User(username=random_uname, password=make_password(random_pwd, None, 'md5'), township=township,
                          type='admin'))
...
def register_new(request):
    ...
    township = Township.objects.create(application_id=application_id, applicant_name=applicant_name,
                                       applicant_phone=applicant_phone, applicant_email=applicant_email,
                                       applicant_designation=applicant_designation, name=name, address=address,
                                       phone=phone, geo_address=geo_address, lat=lat, lng=lng,
                                       verification_link=verification_link, paytm_cust_id=paytm_cust_id)
    ...
    html_content = html.render({'township_name': township.name, 'applicant_name': township.applicant_name,
                                'verification_link': settings.CURRENT_DOMAIN + '/register/verify/' + township.verification_link})

    email_subject = 'New application! (' + township.application_id + ')'
    email = EmailMultiAlternatives(email_subject, 'A new society has submitted an application', settings.DOMAIN_EMAIL,
...
def check_verification(request):
    ...
    try:
        ...
        return JsonResponse(
            [{'request_status': 0, 'status_description': 'Incorrect Application ID and Email combination'}], safe=False)
...
def send_reset_password_link(request):
    ...
    client_email = EmailMultiAlternatives('Reset password', 'To reset your password, click on the following link: ',
                                          settings.DOMAIN_EMAIL, [email])
